...
| --- | --- |
| `ServingStudioSim/` | Simulator, Analyzer, launcher, profiling, and model/GPU definitions |
| `ServingStudioAgent/` | Agent execution, workspaces, conversations, and job lifecycle |
| `ServingStudioUI/app/` | Analysis and Agent browser UI |
| `ServingStudioIntro/` | Introduction website |

- Start with `README.md`; use `reproduce.md` for setup and `ServingStudioSim/doc/README.md`
...
- Before starting any task, you MUST search the available skills for applicable
  ...
  `ServingStudioSim/skills/` and user-installed skills; `skill-of-skills/SKILL.md` maps
  ServingStudio Sim workflows. General-purpose skills belong in the user's skill directory.
- `CLAUDE.md` links to this file. `.agents/skills` and `.claude/skills` link to
  `ServingStudioSim/skills/`. Edit the source, not a separate copy.
...
- Follow each component's environment and test instructions; use `uv` in
  `ServingStudioSim/`. Test the affected behavior and limit formatting to intended files.
